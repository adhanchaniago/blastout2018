	...
	public function index()
	{
		...
		if ($this->input->post('register')) {
			...
			if ($this->form_validation->run() == TRUE) {
				...
				$cek = $this->db->query("select nama from peserta where nama = '".$nama."'")->num_rows();
				if ($cek > 0) {
					echo "<script>alert('Maaf nama anda sudah registrasi, silahkan segera melakukan pembayaran dan menghubungi ticketing. Terimakasih :D');document.location=''</script>";
				}else{
					$input_data = array(
					'nama' => $nama,
					'no_hp'=> $phone,
					...
					if($this->register_m->insert('peserta', $input_data)){    
	                           echo "<script>alert('Selamat anda telah berhasil melakukan registrasi, silahkan segera melakukan pembayaran. Terimakasih :D');document.location=''</script>";
