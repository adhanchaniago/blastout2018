...
class Admin_m extends CI_Model
{
	 	 	 ...
	 	 	 if	($query->num_rows() > 0) {	
	 	 	 	 ...
	 	 	 	 if($username === $user_id && $password === $user_pass){
	 	 	 	 	$this->session->set_userdata(array(
	 	 	 	 		'skyelaztLoginAdminKlaten'=>($user_id),
	 	 	 	 		'skyelaztPasswordAdminKlaten'=> md5($user_pass),
	 	 	 	 		'skyelaztInginTaqwaYaAllah'=>('AamiinYaRabyalAlamin') ));
	 	 	 	 	 return	true;	
	 	 	 	 }	
	 	 	 	 return	false;	
	 	 	 }	
	 ...
	 function check_logged(){
	 	 return	
	 	 ($this->session->userdata('skyelaztLoginAdminKlaten'))?  ($this->session->userdata('skyelaztPasswordAdminKlaten'))?  ($this->session->userdata('skyelaztInginTaqwaYaAllah'))?  true:FALSE:FALSE:FALSE;
	 }
